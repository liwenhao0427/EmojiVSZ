...
*   **英雄单位 (Keyboard Warrior)**:
    ...
    *   能量达到100时，自动释放终极技能 **"ULTIMATE!"**，对全屏所有敌人造成伤害并施加 **冻结** 效果。
...
*   **索敌逻辑**:
    ...
    *   **特殊单位 (MAGIC, 超长射程)**: 可以攻击 **全场** 范围内最近的敌人，无视行限制。
*   **攻击表现**:
    *   **远程/魔法**: 发射与单位 Emoji 相关的弹丸 (Projectile)。魔法弹丸具有追踪能力。
...
*   **升级流程**:
    ...
    *   当 `xp` 达到 `maxXp` 时，等级提升，`xp` 归零，`maxXp` 增加 (`*1.5`)。
...
*   **奖励类型**:
    1.  **临时单位 (Mercenary)**: 获得一个强大的单位，但它只在本波次生效。
    2.  **英雄专属增益 (Hero Buff)**: 为英雄单位提供一个强大的临时 Buff，如“攻击速度翻倍”。
...
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **Keyboard Warrior** | 🦸‍♂️ | MAGIC | - | 10 | 1.0s | 英雄，无限射程，拥有终极技能。 |
...
| :--- | :--- | :--- | :--- |
| 1 | 30s | 3.0s | `troll` |
| 2 | 35s | 2.5s | `troll`, `hater` |
| 3 | 40s | 2.0s | `troll`, `hater`, `bot` |
| 4 | 45s | 1.5s | `hater`, `bot` |
| 5 | 60s | 1.0s | `troll`, `hater`, `bot`, `boss` |
...
*   **检视面板 (Inspector Panel)**:
    ...
    *   **详细数据**: 面板不仅显示最终数值，还将通过 **工具提示 (Tooltip)** 展示伤害和冷却时间的 **计算公式** (例如 `Base: 10, Bonus: +5, Multiplier: x1.20`)，为深度玩家提供决策依据。
